...
(function(){
  ...
  const ASSETS = {
    ...
    cards: {
      ...
      fee: './assets/cards/5_fee_neante.png'
    }
  };
  ...
  const CARD_DEF = {
    ...
    [CardType.FEE]: {
      img: ASSETS.cards.fee,
      entry: null,
      inPlay: null,
      lastBreath: null,
      text: 'En jeu : Cette carte ne peut pas être détruite ou volée.'
    },
  };
  ...
  const DECKLIST = [
    ...
    CardType.MIME, CardType.MIME, CardType.MIME,
    CardType.FEE,
  ...
  // Les attributs booléens HTML (disabled, hidden, etc.)
  // Si false => on ne met PAS l’attribut du tout
...
function destroyCards(ownerTriggering, targets){
  ...
  order.forEach(card => {
    // Immunité : Fée Néante ne peut pas être détruite
    if (card.type === CardType.FEE) {
      logLine(`<span class="muted">${whoLabel(ownerTriggering)} tente de détruire <b>${card.type}</b>, mais elle est intouchable.</span>`);
      flashZone(card.owner, false);
      return;
    }
  ...
  function aiChooseTargets(owner, n){
    ...
    // IA plus compétitive mais encore imparfaite
    const preferOwn = 0.25;    // parfois elle se trompe encore
    const preferRandom = 0.20; // petite imprévisibilité
  ...
  function renderFieldCard(card){
    ...
    const btn = el('button', {
      ...
      el('img', { class:'cardimg', src: def.img, alt: card.type, draggable: 'false' }),
    ]);
  ...
  // 🔍 DEBUG TEMPORAIRE — vérifie que le clic arrive bien
  btn.addEventListener('click', () => {
    console.log('BTN CLICK OK', card.id);
  });
  ...
  function renderAIFaceDown(){
    return el('div', { class:'cardwrap' }, [
      el('button', { class:'cardbtn', disabled: true }, [
        el('img', { class:'cardimg cardimg--faceDown', src: ASSETS.back, alt: 'Dos de carte', draggable: 'false' }),
      ]),
  ...
  function step(){
    ...
    // Player turn (début de tour appliqué une seule fois)
if (!S.turnStarted) {
  S.turnStarted = true;
  S.phase = 'startTurn';
  S.playerPlayedThisTurn = false;

  // En jeu : effets au début de ton tour (Merlin)
  applyStartTurnInPlay('P');

  // Pioche normale : +1 carte
  drawCards('P', 1);

  // Ensuite, tu dois poser 1 carte
  S.phase = 'play';
  updateUI();
  S.lock = false;
  return;
...
// Si le tour est déjà démarré, on ne repioche pas.
// On reste en "play" tant que tu n'as pas posé, sinon on laisse résoudre/fin.
S.phase = S.playerPlayedThisTurn ? 'resolve' : 'play';
updateUI();
S.lock = false;
return;
  }
